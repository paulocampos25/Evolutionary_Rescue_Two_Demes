...
int main(int ac, char **av)
{
  ...
  if (ac < 11 + atoi(av[1]) - 1) {  // Update the condition to reflect the new count
  ...
  sequence.nmut = new int*[popul.n_subpopul];
  for( i=0; i<popul.n_subpopul; i++ )
    sequence.nmut[i] = new int[2*popul.K];
  ...
  sprintf(arq2,"OneStep-Metapopulation-K%d-Lambda%g-ntraits%d-U%g-mig%g-drop%g-Wmax%g.dat",popul.K,popul.lambda,popul.ntraits,popul.mut,popul.migration,drop_fitness[1],sequence.W_max);
  ptt2 = fopen(arq2,"a");
      ...
      for( i=0; i<popul->N[deme]; i++ )
	{
	  ...
	  for( j=0; j<n_offspring; j++ )
		...
		sequence->traits_new[cont][k] = sequence->traits[deme][i][k];
	      ...
	      if( (r < popul->mut) && (sequence->nmut1[cont]==0) )
		{
...
{
  int NDinc[2], nmig[2], k, i, j, ind_group, k_aux, C[20], indice, n_ind, n_migration, **nmut_mig;
  ...
  for( i=0; i<2; i++ )
      ...
      while( n_ind<n_migration )
	{
	  ...
	  nmut_mig[C[i]][NDinc[C[i]]] = sequence->nmut[i][indice];
	  NDinc[C[i]]++;
	  n_ind++;
	  ...
	  for( k=indice; k<(popul->N[i]-1); k++ )
		...
		sequence->traits[i][k][j] = sequence->traits[i][k+1][j];
	      sequence->nmut[i][k] = sequence->nmut[i][k+1];
	    }
	  popul->N[i]--;
	}
  ...
  for( i=0; i<2; i++ )
      ...
      for( j=0; j<NDinc[i]; j++ )
	{
	  ...
	  sequence->nmut[i][popul->N[i]+j] = nmut_mig[i][j];
	}
      popul->N[i] += NDinc[i];
    }
...
void density(sequencia *sequence, populacao *popul, otimo *optimum)
{
    ...
    for (deme = 0; deme < popul->n_subpopul; deme++)
        ...
        if (popul->N[deme] > popul->K)
	  {
		...
		new_nmut[i] =  sequence->nmut[deme][indices[i]];
                for (j = 0; j < popul->ntraits; j++)
		  {
		...
		sequence->nmut[deme][i] = new_nmut[i];
                for (j = 0; j < popul->ntraits; j++)
		  {
