...
int main(int ac, char **av)
{
  ...
  int nwalks, max1, cont_path_total, cont_path, cont_newpath, i, j, k, cont_walk, verif, max, conf, config, t, tmax, model_dominant[2], *cont_seq, int_conf, min_step, cont_max, n_landscapes, popul_max, indice, ind_sub, ind_max, cont_extinction, cont_nonextinction, deme, sum_cont_extinction, cont_ind[2], cont_rescue_total, cont_rescue_partial, cont_loss, cont_fix, nrescue_mut_1, nrescue_mut_2, nrescue_mut_12, cont_mut;

  double fit_max, fit_med, soma1, cont_conf, sum_trait_2, *drop_fitness, *trait, sum_t_2, distance_phen, divergence, f_max, sum_2, y[100], norm, r_mut, trait_mut[50], fitness_mut[5], distance;

  long seed;  //define a seed

  char arq1[1000], arq2[1000], arq3[1000];

  if (ac < 10 + atoi(av[1]) - 1) {  // Update the condition to reflect the new count
  ...
  sequence.label = new int*[popul.n_subpopul];
  for( i=0; i<popul.n_subpopul; i++ )
    sequence.label[i] = new int[2*popul.K];

  sequence.label1 = new int*[popul.n_subpopul];
  for( i=0; i<popul.n_subpopul; i++ )
    sequence.label1[i] = new int[2*popul.K];
  ...
  nrescue_mut_12 = nrescue_mut_1 = nrescue_mut_2 = 0;
  cont_loss = cont_fix = 0;
  while( (++conf)<=config )
    {
      if( (conf%1000)==1 )
	{
	  ...
	  for( deme=0; deme<popul.n_subpopul; deme++ )
	    {
	      ...
	      for( k=0; k<popul.ntraits; k++ )
		optimum.traits[deme][k] = optimum.trait_shift[deme][k]*sqrt(sum_trait_2);
	    }
	}
      ...
      for( deme=0; deme<popul.n_subpopul; deme++ )
	{
	  distance = 0.;
	  for( k=0; k<popul.ntraits; k++ )
	    distance += (optimum.traits[deme][k]-trait_mut[k])*(optimum.traits[deme][k]-trait_mut[k]);
	  
	  fitness_mut[deme] = sequence.W_max*exp(-distance/(2*popul.alpha_r*popul.alpha_r));
	}
      verif = 0;
      if( (fitness_mut[0]>1.) && (fitness_mut[1]>1.) )
	{
	  nrescue_mut_12++;
	  verif = 1;
	}
      ...
      if( verif==1 )
	{
	  ...
	  for( k=0; k<popul.n_subpopul; k++ )
	    for( i=0; i<popul.N[k]; i++ )
	      {
		sequence.label[k][i] = 0;
		for( j=0; j<popul.ntraits; j++ )
		  sequence.traits[k][i][j] = sequence.trait_init[j];
	      }
	  for( j=0; j<popul.ntraits; j++ )
	    sequence.traits[0][0][j] = trait_mut[j];

	  sequence.label[0][0] = 1;
	  popul.nmut = 1;
  ...
  sprintf(arq2,"Fixation-K%d-Lambda%g-ntraits%d-mig%g-drop%g-Wmax%g.dat",popul.K,popul.lambda,popul.ntraits,popul.migration,drop_fitness[1],sequence.W_max);
  ptt2 = fopen(arq2,"a");
  fprintf(ptt2,"%g \t %g \n",drop_fitness[0],((double)cont_fix/n_landscapes));
  fclose(ptt2);
...
void dynamics(sequencia *sequence, populacao *popul, otimo *optimum)
{
  ...
  for( deme=0; deme<popul->n_subpopul; deme++ )
      ...
      for( i=0; i<popul->N[deme]; i++ )
	{
	  ...
	  for( j=0; j<n_offspring; j++ )
		...
		sequence->traits_new[cont][k] = sequence->traits[deme][i][k];
	      sequence->label1[deme][cont] = sequence->label[deme][i];
      ...
      for( i=0; i<popul->N[deme]; i++ )
	{
	  ...
	  sequence->label[deme][i] = sequence->label1[deme][i];
	}
...
{
  int NDinc[2], nmig[2], k, i, j, ind_group, k_aux, C[20], indice, n_ind, n_migration, **label_mig;
  ...
  for( i=0; i<2; i++ )
      ...
      while( n_ind<n_migration )
	{
	  ...
	  label_mig[C[i]][NDinc[C[i]]] =  sequence->label[i][indice];
	  NDinc[C[i]]++;
	  n_ind++;
	  
	  for( k=indice; k<(popul->N[i]-1); k++ )
	    {
	      sequence->label[i][k] = sequence->label[i][k+1];
	      for( j=0; j<popul->ntraits; j++ )
  ...
  for( i=0; i<2; i++ )
    {
      for( j=0; j<NDinc[i]; j++ )
	{
	  sequence->label[i][popul->N[i]+j] = label_mig[i][j];
	  for( k=0; k<popul->ntraits; k++ )
...
void density(sequencia *sequence, populacao *popul, otimo *optimum)
{
    ...
    for (deme = 0; deme < popul->n_subpopul; deme++)
        ...
        if (popul->N[deme] > popul->K)
	  {
	    ...
	    std::vector<double> new_label(popul->K);

            for (int i = 0; i < popul->K; ++i)
	      {
                new_fitness[i] = sequence->fitness[deme][indices[i]];
		new_label[i] = sequence->label[deme][indices[i]];
                for (j = 0; j < popul->ntraits; j++)
		  {
		...
		sequence->label[deme][i] = new_label[i];
                for (j = 0; j < popul->ntraits; j++)
		  {
